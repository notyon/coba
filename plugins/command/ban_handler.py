...
async def ban_handler(client: Client, msg: types.Message):
    if re.search(r"^[\/]ban(\s|\n)*$", msg.text):
            ...
            await client.send_message(
                chat_id=config.channel_1,
                text=f"🚫 User <code>{target}</code> telah dibanned.\nAlasan: <b>{' '.join(msg.text.split()[2:]) or '-tanpa alasan-'}</b>",
                parse_mode=enums.ParseMode.HTML
            )
            
            try:
                await client.ban_chat_member(config.channel_2, int(target))
            except Exception as e:
                await msg.reply(f"Gagal ban user dari grup: {e}")
